...
- **Parents & Child list** — the master roster (searchable, status-filterable).
...
The app has two plain-English, screenshot-led help pages (same pattern as the
sibling **student-attendance** app):

- **`/help`** — public parent guide (linked as **Help** in the top nav):
  registering, paying, retrieving the update link, and editing a registration.
- **`/admin/help`** — staff guide (behind login, in the **Admin** menu): the
  family lists, allergies/medical, class placement, payment override, and CSV
  import/export.

Each is a static Blade view (`resources/views/help/{parent,admin}.blade.php`).
The screenshots live in `public/images/help/{parent,admin}/` and are generated
by a Dusk capture (not a regression test), guarded so it never runs in CI. To
